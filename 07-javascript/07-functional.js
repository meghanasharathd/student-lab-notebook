...
const mapNameFamily = (characters) => {
 ...
 characters =characters.map(character => `${character.name}, of House ${character.house}`)
  return characters;  
}
...
const reduceHouses = (characters) => {
  ...
  return characters.reduce((num, character) => {
    num[character.house] = (num[character.house] || 0) + 1;
    return num;
  }, {});
};
